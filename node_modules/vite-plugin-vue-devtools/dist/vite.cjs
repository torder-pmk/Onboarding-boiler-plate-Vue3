...
const path$3__default = /*#__PURE__*/_interopDefaultCompat(path$3$1);
...
var queueMicrotask_1 = typeof queueMicrotask === 'function'
  ? queueMicrotask.bind(typeof window !== 'undefined' ? window : commonjsGlobal$1)
...
async function getStaticAssets(config) {
  ...
  cache = await Promise.all(files.map(async (path) => {
    ...
    const stat = await fs__default.lstat(filePath);
    const publicDirname = path$3__default.relative(config.root, config.publicDir);
    const normalizedPath = publicDirname === path$3__default.basename(path$3__default.dirname(path)) ? path.replace(publicDirname, "") : path;
    return {
      path: normalizedPath,
      filePath,
      publicPath: join$1(baseURL, normalizedPath),
      type: guessType(path),
      size: stat.size,
      mtime: stat.mtimeMs
    };
...
async function getImageMeta(filepath) {
  ...
  try {
    const meta = imageMeta(await fs__default.readFile(filepath));
...
async function getTextAssetContent(filepath, limit = 300) {
  try {
...
async function deleteStaticAsset(filepath) {
  try {
    return await fs__default.unlink(filepath);
  } catch (e) {
    console.error(e);
    throw e;
  }
}
async function renameStaticAsset(oldPath, newPath) {
  const exist = cache?.find((asset) => asset.filePath === newPath);
  if (exist)
    throw new Error(`File ${newPath} already exists`);
  return await fs__default.rename(oldPath, newPath);
}
...
async function locatePath(
	...
	const statFunction = allowSymlinks ? fs$1$1.promises.stat : fs$1$1.promises.lstat;

	return pLocate(paths, async path_ => {
		try {
			const stat = await statFunction(path$3__default.resolve(cwd, path_));
...
async function findUpMultiple(name, options = {}) {
	let directory = path$3__default.resolve(toPath(options.cwd) || '');
	const {root} = path$3__default.parse(directory);
	const stopAt = path$3__default.resolve(directory, options.stopAt || root);
	...
	while (true) {
		...
		if (foundPath) {
			matches.push(path$3__default.resolve(directory, foundPath));
		}
		...
		directory = path$3__default.dirname(directory);
	}
...
function npmRunPath(options = {}) {
	const {
		cwd = process$2__default.cwd(),
		path: path_ = process$2__default.env[pathKey()],
		execPath = process$2__default.execPath,
	} = options;

	let previous;
	const cwdString = cwd instanceof URL ? url__default.fileURLToPath(cwd) : cwd;
	let cwdPath = path$3__default.resolve(cwdString);
	const result = [];

	while (previous !== cwdPath) {
		result.push(path$3__default.join(cwdPath, 'node_modules/.bin'));
		previous = cwdPath;
		cwdPath = path$3__default.resolve(cwdPath, '..');
	}

	// Ensure the running `node` binary is used.
	result.push(path$3__default.resolve(cwdString, execPath, '..'));

	return [...result, path_].join(path$3__default.delimiter);
}

function npmRunPathEnv({env = process$2__default.env, ...options} = {}) {
	env = {...env};
...
if (!processOk(process$1)) {
  ...
  var assert = require$$0$3__default;
...
const isSigterm = signal => signal === os$1__default.constants.signals.SIGTERM
...
const pipeToTarget = (spawned, streamName, target) => {
	if (typeof target === 'string') {
		spawned[streamName].pipe(fs$1$1.createWriteStream(target));
		return spawned;
	}
...
const getInput = ({input, inputFile}) => {
	...
	return fs$1$1.createReadStream(inputFile);
};
...
const logCommand = (escapedCommand, {verbose}) => {
	...
	process$2__default.stderr.write(`[${getTimestamp()}] ${escapedCommand}\n`);
};

const DEFAULT_MAX_BUFFER = 1000 * 1000 * 100;

const getEnv = ({env: envOption, extendEnv, preferLocal, localDir, execPath}) => {
	const env = extendEnv ? {...process$2__default.env, ...envOption} : envOption;
...
const handleArguments = (file, args, options = {}) => {
	...
	options = {
		...
		extendEnv: true,
		preferLocal: false,
		localDir: options.cwd || process$2__default.cwd(),
		execPath: process$2__default.execPath,
		encoding: 'utf8',
		reject: true,
		cleanup: true,
	...
	if (process$2__default.platform === 'win32' && path$3__default.basename(file, '.exe') === 'cmd') {
		// #116
		args.unshift('/q');
	}
...
async function detect({ autoInstall, programmatic, cwd } = {}) {
  ...
  let packageJsonPath;
  if (lockPath)
    packageJsonPath = path$3__default.resolve(lockPath, "../package.json");
  else
    packageJsonPath = await findUp("package.json", { cwd });
  if (packageJsonPath && fs$1__default.existsSync(packageJsonPath)) {
    try {
      const pkg = JSON.parse(fs$1__default.readFileSync(packageJsonPath, "utf8"));
...
function VitePluginVueDevTools(options) {
  ...
  function configureServer(server) {
    ...
    server.watcher.on("all", (event, path2) => {
      rpc.onFileWatch({ event, path: path2 });
    });
  }
  const plugin = {
...
import 'virtual:vue-devtools-path:app.js'`;
      return compiler.analyzeCode(code, id, analyze);
    },
